...
type AssinaturaPaymentRouteProp = RouteProp<ProfileStackParamList, 'AssinaturaPayment'>;
// RootStackParamList: permite voltar para Main → HomeStack ao finalizar.
...
/**
 * Pagamento da assinatura (fluxo simulado, igual ao PaymentScreen da sessão).
 * Estrutura idêntica: cartão de crédito + PIX + processando + sucesso.
 * Ao confirmar, tenta ativar a assinatura no backend; se o endpoint ainda não
 * existir, o erro é tratado silenciosamente e o sucesso é exibido mesmo assim
 * (pois é uma simulação até a integração real com RevenueCat).
 */
export const AssinaturaPaymentScreen = () => {
  const { colors } = useTheme();
  const styles = React.useMemo(() => createStyles(colors), [colors]);
  const navigation = useNavigation<NavigationProp>();
  const route = useRoute<AssinaturaPaymentRouteProp>();
  const { planoNome, planoPreco } = route.params;
  ...
  const processarPagamento = async () => {
    ...
    try {
      const confirmar = async () => {
        try {
          // Tenta ativar a assinatura no backend. Planos mock usam o nome
          // como identificador. Se o endpoint não existir (404/500), o erro
          // é tratado silenciosamente — fluxo simulado.
          await api.post('/api/assinatura/ativar', { planoId: planoNome });
        } catch {
          // Silencioso: exibimos sucesso de qualquer forma (simulação).
        }
        setEtapa('sucesso');
        setLoading(false);
      };
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        >
          {/* Cabeçalho com o plano escolhido */}
          <View style={styles.header}>
            <Text style={styles.headerTitle}>Assinatura Premium</Text>
            <Text style={styles.headerPlano}>{planoNome}</Text>
            <Text style={styles.headerValor}>{planoPreco}</Text>
          </View>
          ...
          {etapa === 'sucesso' ? (
            <View style={styles.sucessoContainer}>
              ...
              />
              <Text style={styles.sucessoTitulo}>Assinatura ativada!</Text>
              <Text style={styles.sucessoMensagem}>
                Bem-vindo ao Cedro Premium! Você já pode aproveitar reuniões ilimitadas e todos os
                benefícios do plano.
              </Text>
              <Button
              ...
              {metodo === 'pix' && mostrarFormulario && pixCode && (
                <View style={styles.pixContainer}>
                  ...
                  <Button
                    ...
                    style={styles.botaoConfirmar}
                  />
                  <Button title="Confirmar pagamento" onPress={processarPagamento} />
                </View>
              )}
            </>
          )}
